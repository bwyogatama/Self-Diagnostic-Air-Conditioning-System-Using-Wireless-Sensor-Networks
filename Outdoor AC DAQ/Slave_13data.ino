...
#define ONE_WIRE_BUS 8       // Data wire is plugged into pin 9 on the Arduino
...
DeviceAddress CompTemp = {0x28, 0xFF, 0x46, 0xD0, 0xC1, 0x17, 0x04, 0x27};
...
float outdoor_temp;
float con_inlet_temp;
float con_outlet_temp;
float compressor_temp;
...
void setup()   /****** SETUP: RUNS ONCE ******/
{
  ...
  sensors.setResolution(CompTemp,10);
...
void loop()   /****** LOOP: RUNS CONSTANTLY ******/
{
  ...
  compressor_temp     = sensors.getTempC(CompTemp);
  ...
  Serial.println(RS485Serial.write(payload,24));  // Send byte to Remote Arduino
